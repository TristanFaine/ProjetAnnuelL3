...
header("Access-Control-Allow-Methods: PUT"); // méthode accepté pour la requete pour insérer
...
if($_SERVER['REQUEST_METHOD'] == 'PUT'){
    ...
    $donnees = new Donnees($db);

    // On récupère les informations envoyées
    $Donnees = json_decode(file_get_contents("php://input"));

    // Ici on a reçu les données
    if(!empty($Donnees->status) && !empty($Donnees->entryPoint) && !empty($Donnees->begin) && !empty($Donnees->end) ){
    
        //On met les informations dans l'objet donnees
        $donnees->status = $Donnees->status;
        $donnees->entryPoint = $Donnees->entryPoint;
        $donnees->begin = $Donnees->begin;
        $donnees->end = $Donnees->end;
        if($donnees->modifier()){ // Ici la modification a fonctionné
            
           
            http_response_code(201);  // On envoie un code 201
            echo json_encode(["message" => "La modification a été effectuée"]);

        }else // si la modification n'a pas fonctionné
